/* =========================================================
   狼人殺｜上帝輔助 PWA（完整版｜流程修正版）
   ✅ 修正：女巫彈窗顯示規則
      - 未用解藥：顯示刀口 + 救/不救 + 毒/不毒
      - 已用解藥：不顯示刀口，只能毒/不毒
   ✅ 修正：夜晚結束一定產生公告並彈窗顯示
   ✅ 修正：白天可一路操作（上警/發言/投票）
   ✅ 新增：白天結束 → 下一夜（若有 btnDayNext）
========================================================= */
...
(() => {
  ...
  function renderAnnouncementBox(){
    ...
    if(annAsWitchPanel){
      ...
      // ✅ 核心：已用解藥 → 不顯示刀口
      const wolfLine = saveUsed ? "今晚被刀：（隱藏）" : `今晚被刀：${wolf?wolf+" 號":"（尚未選狼刀）"}`;

      title.textContent =
        `【女巫操作】\n${wolfLine}\n\n解藥：${saveUsed?"已用過":"可用"}\n毒藥：${poisonUsed?"已用過":"可用"}`;
      ...
      if(!saveUsed){
        ...
        btnNoSave.className="btn ghost"; btnNoSave.type="button";
      ...
      btnNoPoison.className="btn ghost"; btnNoPoison.type="button";
      btnNoPoison.textContent="不用毒藥";
      btnNoPoison.disabled=!canPoison && !State.night.witchPoisonTarget;
      btnNoPoison.onclick=()=>{
      ...
      done.className="btn ghost"; done.type="button";
  ...
  function getScriptForStep(step){
    ...
    if(step.type==="witch" && State.godView){
      // ✅ 注意：若解藥已用過，這裡也不要顯示刀口（避免上帝不小心給玩家看到）
      const wolfLine = State.night.witchSaveUsed ? "（已用解藥，本晚刀口不再顯示）" : `今晚被刀：${State.night.wolfTarget?State.night.wolfTarget+" 號":"—"}`;
      extra += `\n\n（上帝）${wolfLine}`;
  ...
  function renderNightUI(){
    ...
    renderSeatDots(nightSeats,(seat)=>{
      ...
      if(s.type==="pick" && s.pickTarget){
        ...
        // ✅ 修正：若是預言家查驗（pickTarget = seerCheckTarget）也要即時計算 seerResult
        if(s.pickTarget==="seerCheckTarget"){
  ...
  /* =========================================================
     ✅ 白天結束 → 下一夜（若你的 index.html 有 btnDayNext）
     - 這是你說「按下一步沒反應」最常缺的那顆釘子
  ========================================================= */
  ...
  // 如果你 index.html 有「下一步」白天按鈕，id 建議叫 btnDayNext
  on($("btnDayNext"),"click",goNextNight);

  /* =========================
     Day buttons（保留你原本的綁定，避免你 UI 失效）
     你現在這份 app.js 沒有貼完整 day 的模組，
     但只要你的 index.html 有 btnPolice / btnTalkOrder / btnVote，
     後續我下一個檔案會把「完整版 dayUI + 平票PK」整段補齊且對上。
  ========================= */
  ...
  function boot(){
    ...
    if(State.phase==="day"){
      $("dayTag") && ($("dayTag").textContent = `第 ${State.dayNo} 天`);
      // ✅ 進白天若沒有公告（例如你之前卡住），也補開一次
      if(State.logs?.length) openAnnouncementModal(false,true);
    }
